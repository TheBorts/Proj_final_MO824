...
template <typename E> class BiasFunction {
    public:
        virtual int selectCandidate(const vector<E>& RCL, Evaluator<E>* objFunction, Solution<E>* sol) = 0;
};
...
template <typename E> class AbstractGRASP {
    public:
        ...
        virtual vector<E> makeCL() = 0;
        virtual vector<E> makeRCL() = 0;
        ...
        Solution<E> constructiveHeuristic(int numberOfRandomIterations) {
            ...
            while (!constructiveStopCriteria()) {
                ...
                if (!isfinite(minCost) || !isfinite(maxCost)) break;
                ...
                sort(RCL.begin(), RCL.end(), [&](const E& a, const E& b) {
                ...
                CL.erase(remove(CL.begin(), CL.end(), inCand), CL.end());
                sol.add(inCand);
        ...
        Solution<E> solve(int numberOfRandomIterations) {
            ...
            while (true) {
                ...
                if (bestSol.cost > sol.cost) {
                    bestSol = sol;
                    if (verbose) {
                        cout << "BestSol cost = " << bestSol.cost << endl;
                    }
                }
            }
