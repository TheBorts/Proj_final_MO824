...
def save_results_to_csv(results, filename):
    ...
    with open(filename, 'w', newline='') as csvfile:
        ...
        writer.writerow(['Instance', 'Num_Points', 'Num_Clusters', 'Method', 'Execution_Time', 'Cost', 'Medoids'])
    ...
    for instance_name in instances_names:
        ...
        for num_clusters in number_of_clusters:
            for method in methods:
                ...
                results.append([instance_name, num_points, num_clusters, method, execution_time, cost, medoids])
                print(f"Instance: {instance_name}, Points: {num_points}, Clusters: {num_clusters}, Method: {method}, Time: {execution_time:.4f}, Cost: {cost}")
    save_results_to_csv(results, "../results/clustering_results.csv")